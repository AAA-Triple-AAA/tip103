...
Christopher Robin set up a scavenger hunt for Pooh, but it's a blustery 
day and several hidden clues have blown away. Write a function 
...
A clue x is considered missing if x is in the range [lower, upper] and x 
is not in clues.
...
def find_missing_clues(clues: list[int], lower: int, upper: int) -> list[list[int]]:
    ...
    for i in range(len(clues)):
        low = lower if i == 0 else clues[i-1]
