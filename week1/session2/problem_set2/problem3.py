...
parameter and returns a new string with each substring with 
consecutive spaces reduced to a single space. Assume s can contain 
leading or trailing spaces, but in the result should be trimmed. 
...
def squash_spaces(s: str) -> str:
    lst = s.split(" ")
    new_lst = [ch for ch in lst if ch]
    new_s = " ".join(new_lst)
